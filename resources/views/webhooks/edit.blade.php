...
<div class="grid grid-cols-12 gap-4><div class="col-span-12 md:col-span-8"><div class="bg-white rounded-xl shadow-md border border-gray-200 dark:bg-gray-800 dark:border-gray-700"><div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700"><h3 class="font-semibold text-gray-900 dark:text-white">Edit Webhook</h3></div>
    <form action="{{ route('webhooks.update', $webhook) }}" method="POST">@csrf @method('PUT')
        <div class="p-6">
            ...
            <div class="mb-4"><div class="icheck-primary"><input type="checkbox" name="is_active" id="is_active" value="1" {{ $webhook->is_active ? 'checked' : '' }}><label for="is_active">Active</label>
        </div>
        <div class="card-footer"><button class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 inline-flex items-center gap-2 font-medium transition-colors">Update</button> <a href="{{ route('webhooks.show', $webhook) }}" class="btn btn-default">Cancel</a></div>
    </form>
</div>
</div>
@endsection
