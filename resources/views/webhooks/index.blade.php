            ...
            <tbody>
                @forelse($webhooks as $webhook)
                    <tr>
                        ...
                        <td><small class="text-gray-500 dark:text-gray-400">{{ Str::limit($webhook->url, 40) }}</small></td>
                        <td>{{ count($webhook->events ?? []) }} events</td>
                        <td>{{ $webhook->total_calls }} ({{ $webhook->failed_calls }} failed)</td>
                        <td><span class="{{ $webhook->is_active ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300' }} text-xs font-medium px-2.5 py-0.5 rounded-full">{{ $webhook->is_active ? 'Active' : 'Inactive' }}</span></td>
                        <td>
                            <a href="{{ route('webhooks.edit', $webhook) }}" class="px-4 py-2 border border-yellow-300 dark:border-yellow-600 text-yellow-600 dark:text-yellow-400 rounded-lg hover:bg-yellow-50 dark:hover:bg-yellow-900/20 text-sm font-medium transition-colors inline-flex items-center gap-1"><i class="fas fa-edit"></i></a>
                ...
                @empty
                    <tr><td colspan="6" class="text-center text-gray-500 dark:text-gray-400">No webhooks configured</td></tr>
                @endforelse
            </tbody>
        </table></div>
    </div>
